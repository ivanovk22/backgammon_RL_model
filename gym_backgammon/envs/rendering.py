...
class Viewer(object):
    def __init__(self, width, height):
        ...
        self.dice_label = pyglet.text.Label('',
                                            font_name='Arial',
                                            font_size=24,
    ...
    def render(self, board, bar, off, state_w, state_h, roll=None, return_rgb_array=True):
        ...
        # --- DICE RENDERING SECTION ---
        dice_sprites = []
        if roll:
            clean_roll = [abs(d) for d in roll]

            if any(d < 0 for d in roll):  # WHITE rolls are negative in this env
                self.dice_label.text = f" WHITE PLAY: {clean_roll[0]}  {clean_roll[1]}"
                self.dice_label.color = (200, 0, 0, 255)  # Black
            else:
                self.dice_label.text = f" BLACK PLAY: {clean_roll[0]}  {clean_roll[1]}"
                self.dice_label.color = (200, 0, 0, 255)  # Dark Red
        else:
            self.dice_label.text = ""  # Clear it if no roll
        ...
        if return_rgb_array:
            ...
            arr = arr.reshape((state_h, state_w, 3))
            arr = arr[::-1, :, 0:3]
        gl.glViewport(0, 0, SCALING * self.window.width, SCALING * self.window.height)
        pyglet.sprite.Sprite(img=self.empty_board_image, batch=None).draw()
        batch.draw()

        # Draw the dice label last so it's on top
        self.dice_label.draw()
        self.window.flip()
